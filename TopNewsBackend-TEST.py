...
def checkFinanceWidgetVisiblity():
    ...
    if ((currentTime.isoweekday() == 6 and currentTime.hour >= 6) or (currentTime.isoweekday() == 7) or (currentTime.isoweekday() == 1 and currentTime.hour < 6)):
...
def updateNewsTicker():
    ...
    newsHeadlines = []
    excludedCats = ["體育", "資訊節目（新聞）", "推廣", "新聞通識", "資訊節目（財經）", "兩岸國際", "中國在線", "undefined"]
    ...
    for news in jsonObject['data']['posts']['edges']:
        ...
        if not (any(ext in headlineString for ext in filteredKeywords)):
            ...
            if (headerTextDisplayed == False):
                headlineString = headerText + headlineString
